...
const PROJECTS = [
  ...
  { id:"irplays",     sev:9, cat:"SOC/DFIR", year:2025, title:"IR Playbooks — 3 Scenarios",                tags:["NIST IR","D3FEND","Ransomware"],                  summary:"Clinic malware, CityWorks ransomware, Bazaarjo supply-chain. ATT&CK→D3FEND. FIDO2 MFA breaks Lumma.", highlights:["Ransomware decision tree","D3FEND: FIDO2 breaks Lumma","Exec update + advisory","Closure criteria defined"], github:null },
...
function Panel({ children, style={}, glow=false }) {
  ...
  return (
    <div style={{ position:"relative", background:T.panel, border:`1px solid ${bc}`, ...style }}>
      <div style={{ ...br, top:-1, left:-1, borderWidth:"1.5px 0 0 1.5px" }} />
      <div style={{ ...br, top:-1, right:-1, borderWidth:"1.5px 1.5px 0 0" }} />
      <div style={{ ...br, bottom:-1, left:-1, borderWidth:"0 0 1.5px 1.5px" }} />
      <div style={{ ...br, bottom:-1, right:-1, borderWidth:"0 1.5px 1.5px 0" }} />
      {children}
    </div>
  );
}
...
// ── GRID BACKGROUND — Map Room sharp grid + glow ─────────────
function GridBg() {
  return (
    <div style={{ position:"fixed", inset:0, zIndex:0, pointerEvents:"none", overflow:"hidden" }}>
      ...
      {/* Topo curves layered on top — subtle depth */}
    ...
    <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:800, transition:"all 0.4s", background:atTop?"transparent":"rgba(8,12,20,0.92)", backdropFilter:atTop?"none":"blur(20px)", borderBottom:atTop?"none":`1px solid ${T.border}` }}>
      <div style={{ maxWidth:1240, margin:"0 auto", padding:"0 clamp(16px,5vw,60px)", height:56, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <button onClick={()=>setMob(o=>!o)} className="mob-btn"
          style={{ display:"none", background:"none", border:`1px solid ${T.border}`, color:T.text, padding:"5px 11px", fontFamily:T.mono, fontSize:"0.7rem", cursor:"pointer" }}>
          {mob?"✕":"☰"}
        </button>
      </div>
      {mob&&(
...
function HomeView() {
  ...
  return (
    <section style={{ minHeight:"100vh", display:"flex", flexDirection:"column", justifyContent:"center", padding:"100px clamp(16px,5vw,60px) 80px", position:"relative", overflow:"hidden" }}>
      <div style={{ maxWidth:1240, margin:"0 auto", width:"100%", position:"relative", zIndex:2 }}>
        ...
        <div style={a(2)}>
          ...
          <h1 className="hero-name" style={{ fontFamily:T.syne, fontWeight:800, color:T.text, lineHeight:0.88, letterSpacing:"-0.03em", marginBottom:28, position:"relative" }}>
            FAROUQ<br/>
            <span style={{ WebkitTextStroke:`2px ${T.cyan}`, color:"transparent" }}>HASSAN</span>
          </h1>
        </div>
      ...
      <div style={{ maxWidth:1240, margin:"0 auto", position:"relative", zIndex:2 }}>
        ...
        <Reveal>
          ...
          <div style={{ display:"flex", gap:5, flexWrap:"wrap", marginBottom:24, alignItems:"center" }}>
            <span style={{ fontFamily:T.mono, fontSize:"0.52rem", color:T.textDim, letterSpacing:"0.1em" }}>MIN SEV:</span>
            {[[0,"ALL"],[5,"MED+"],[7,"HIGH+"],[9,"CRIT"]].map(([s,l])=>(
              <button key={s} onClick={()=>setMinSev(s)}
                style={{ padding:"3px 9px", background:minSev===s?"rgba(255,255,255,0.07)":"transparent", border:`1px solid ${minSev===s?T.borderHi:T.border}`, color:minSev===s?T.text:T.textDim, fontFamily:T.mono, fontSize:"0.54rem", cursor:"pointer", transition:"all 0.16s" }}>
                {l}
              </button>
            ))}
          </div>
...
function CertsView() {
  ...
  return (
    <section style={{ padding:"110px clamp(16px,5vw,60px)", position:"relative" }}>
      <div style={{ maxWidth:1240, margin:"0 auto", position:"relative", zIndex:2 }}>
        <SectionHead number={4} label="CREDENTIALS" title="Clearance Registry" />
...
function WriteupsView() {
  return (
    <section style={{ padding:"110px clamp(16px,5vw,60px)", position:"relative" }}>
      <div style={{ maxWidth:1240, margin:"0 auto", position:"relative", zIndex:2 }}>
        <SectionHead number={5} label="INTELLIGENCE REPORTS" title="Field Dispatches" />
        {WRITEUPS.map((w,i)=>(
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        /* ── HERO NAME — desktop big, mobile fits ── */
        ...
        .signal-mob   { display:none  !important; }
        ...
        @media(max-width:700px) {
          ...
          /* Mobile name: 12vw at 390px = 46.8px — FAROUQ (6 chars) fits cleanly */
