...
function Panel({ children, style={}, glow=false }) {
  ...
  return (
    <div
      ...
      style={{ position:"relative", background:T.panel, border:`1px solid ${hov?T.cyan+"50":bc}`, boxShadow:hov?`0 0 20px rgba(0,212,255,0.06)`:"none", transition:"border-color 0.25s, box-shadow 0.25s", ...style }}>
...
function CursorTrail() {
  ...
  useEffect(() => {
    const move = (e) => {
      const newDot = { id: id.current++, x: e.clientX, y: e.clientY };
      setDots(prev => [...prev.slice(-18), newDot]);
    };
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:999, pointerEvents:"none" }}>
      {dots.map((dot, i) => {
        const age = dots.length - 1 - dots.findIndex(d => d.id === dot.id);
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        /* Glowing cyan pulse on the hero name */
        ...
        .hero-name { animation: nameGlow 4s ease infinite; }
