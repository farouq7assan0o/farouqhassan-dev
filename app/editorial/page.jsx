...
const PROJECTS = [
  { id:"animeblast", title:"AnimeBlast — Full-Scope Penetration Test", category:"pentest", year:"2024", tags:["Buffer Overflow","Metasploit","SQLi","XSS","RCE","Pivoting"], summary:"Full-scope pentest against two virtual targets. Custom Python BoF exploit (EIP at 1036 bytes), bypassed DEP/ASLR/SafeSEH, pivoted through a compromised host via SOCKS proxy, then exploited UNION SQLi, XSS session hijack, PHP shell upload → RCE on the second target. Captured 10 CTF flags.", highlights:["Custom BoF — EIP @ 1036 bytes","DEP + ASLR + SafeSEH bypass","Network pivot via Metasploit SOCKS","UNION SQLi → credentials dumped","PHP shell → RCE → admin user created","10 flags captured (Base64 + RC4 decoded)"], githubUrl:null },
  { id:"cyberblast", title:"CyberBlast — Ethical Hacking Assessment", category:"pentest", year:"2024", tags:["Nessus","SQLmap","Hydra","EternalBlue","Kill Chain"], summary:"Full-cycle ethical hacking engagement. Passive recon (WHOIS, Google Dorks, Shodan), Nessus scan (20 findings), Wireshark credential capture via plaintext Telnet, SQLmap DB dump, XSS session hijack, command injection, EternalBlue exploitation with Meterpreter post-exploitation and RDP access.", highlights:["Nessus: 20 vulns (BlueKeep, TLS 1.0, PHP 5.6)","EternalBlue → Meterpreter → credential dump","UNION SQLi + XSS + RCE + command injection","Custom MSFvenom reverse shell payload"], githubUrl:null },
  { id:"sqli-redteam", title:"SQLi Assessment + Banking Red Team Plan", category:"pentest", year:"2025", tags:["SQLi","Auth Bypass","UNION injection","Kill Chain","ROE"], summary:"Three parts: manual SQLi exploitation (auth bypass, category filter, UNION credential extraction), full Rules of Engagement for a banking red team, and a complete Cyber Kill Chain attack plan from OSINT through C2 and exfiltration.", highlights:["Auth bypass via SQL injection","UNION SELECT credential extraction","Banking ROE: scope, social engineering limits, data handling","Kill Chain: OSINT → weaponize → phish → persist → C2 → exfil"], githubUrl:null },
  { id:"ftp-brute", title:"FTP Enumeration & Brute Force Lab", category:"pentest", year:"2025", tags:["Hydra","FTP","Brute Force","Enumeration"], summary:"Enumerated FTP, confirmed anonymous access misconfiguration, brute-forced credentials using Hydra against 100k NCSC password list. Documented hardening: Fail2Ban, SFTP migration, firewall ACLs.", highlights:["Anonymous FTP access confirmed","Hydra + 100k NCSC wordlist","No rate limiting — zero lockout resistance","Remediation: Fail2Ban + disable anon + SFTP"], githubUrl:null },
  { id:"privesc-lab", title:"Linux File Transfer & Privilege Escalation", category:"pentest", year:"2025", tags:["LinPEAS","Kernel Exploit","Netcat","SCP","Searchsploit"], summary:"Demonstrated four file transfer techniques (HTTP/Python server, SCP, Netcat, Base64), enumerated with LinPEAS, identified kernel 2.6.32, found Exploit-DB 18411, compiled and executed local privilege escalation.", highlights:["Kernel 2.6.32 identified via uname -a","searchsploit → Exploit-DB 18411","BoF LPE compiled + executed on target","4 transfer methods demonstrated"], githubUrl:null },
  { id:"semgrep", title:"Semgrep SAST in GitHub Actions (Juice Shop)", category:"pentest", year:"2025", tags:["Semgrep","SAST","GitHub Actions","DevSecOps","CI/CD Security"], summary:"Integrated Semgrep into GitHub Actions CI/CD for OWASP Juice Shop. Pipeline auto-scans every push — 1015 files, 1062 rules, 41 findings. Detected direct SQL string concatenation in Sequelize queries.", highlights:["41 findings across 1015 files","Sequelize SQLi: criteria directly concatenated","YAML workflow triggers on every push","Parameterized query remediation documented"], githubUrl:null },
  { id:"malware", title:"Malware Analysis — BackdoorBeacon.exe", category:"pentest", year:"2024", tags:["IDA Free","x32dbg","Wireshark","UPX","Reverse Engineering"], summary:"Full lifecycle RE of a UPX-packed backdoor. Static analysis: TLS callbacks, anti-debug checks, hardcoded C2 IP, HKCU Run persistence. Dynamic: patched C2 IP, confirmed SYN beaconing via Wireshark. SSDT/IDT clean — no rootkit.", highlights:["UPX unpacking → PE section analysis","TLS callback + IsDebuggerPresent NOP patch","C2 IP patched to 127.0.0.1 for safe detonation","Registry persistence: HKCU\\Run\\Updater","SSDT/IDT clean — no kernel hooking"], githubUrl:null },
  { id:"pcap-memory", title:"PCAP + Memory Forensics Investigation", category:"soc", year:"2025", tags:["Wireshark","Volatility 3","DNS Tunneling","HTTP C2","Fileless Malware"], summary:"Two-part investigation: PCAP triage found HTTP C2 beaconing via /v1/checkin with sequential polling, plus DNS TXT exfiltration with Base64 payloads. Memory forensics (Volatility 3) found fileless PowerShell loader, rogue process, active C2 connection.", highlights:["HTTP C2: /v1/checkin sequential polling","DNS TXT exfil with Base64 encoded payload","Volatility: RWX shellcode injection via malfind","Rogue process: parent explorer.exe","Timeline correlated: network + memory"], githubUrl:null },
  { id:"threat-intel", title:"Threat Intelligence — APT29 & Lumma Stealer", category:"soc", year:"2025", tags:["MITRE ATT&CK","ATT&CK Navigator","OSINT","APT29 / NOBELIUM","Lumma Stealer"], summary:"Mapped APT29 (NOBELIUM) SolarWinds and USAID campaigns to ATT&CK with impact scoring. Built Navigator JSON layers with OSINT-backed evidence. Analyzed Lumma Stealer IOCs and behavioral chain. Cross-campaign comparison: nation-state espionage vs cybercrime credential theft.", highlights:["APT29: T1195.002 supply chain — High impact","T1003.001 LSASS dump + T1550.001 token abuse: High","Lumma: T1555 credential harvesting","Full ATT&CK Navigator JSON layers built","CISA AA20-352A + Mandiant SUNBURST sourced"], githubUrl:null },
  { id:"ir-playbooks", title:"IR Playbooks — Clinic, Ransomware, Supply Chain", category:"soc", year:"2025", tags:["IR Planning","ATT&CK → D3FEND","Ransomware","NIST IR","Executive Comms"], summary:"Three complete IR playbooks: clinic malware, CityWorks ransomware (decision tree, recovery preconditions), and Bazaarjo supply-chain breach (NIST IR phase mapping, ATT&CK → D3FEND countermeasures). All include executive comms drafts and closure criteria.", highlights:["VLAN ACL: balanced speed + evidence preservation","Ransomware decision tree: pre-impact vs ongoing","D3FEND: FIDO2 MFA breaks Lumma credential chain","All playbooks: exec update + staff advisory + closure criteria"], githubUrl:null },
  { id:"forensics", title:"Digital Forensics — BlackEagle Investigation", category:"soc", year:"2024", tags:["FTK Imager","HxD","NTFS Recovery","Steganography","Chain of Custody"], summary:"Full forensic investigation of a damaged USB and suspect PC. Manually recovered NTFS mirror via HxD, extracted a hidden DOCX inside a PNG (steganography), decoded concealed text revealing a planned criminal meeting. Hash verified at every stage, full chain of custody.", highlights:["NTFS manual recovery via HxD hex editor","Steganography: DOCX embedded inside PNG","Hidden message decoded — planned meeting details","Hash verified before + after imaging","Search warrant, CCPA/GDPR compliance enforced"], githubUrl:null },
  { id:"airline", title:"Secure Airline Check-in System", category:"soc", year:"2024", tags:["Java","SHA-256","JUnit","Fuzz Testing","RBAC"], summary:"Java airline management system with security-first design: SHA-256 + salt hashing, brute-force lockout after 3 failures, RBAC with 4 roles, centralized audit logger, full JUnit suite with fuzz testing up to 100k login attempts, and SAST via PMD.", highlights:["SHA-256 + salt hashing","Lockout after 3 failures","RBAC: 4 strictly separated roles","Fuzz tested: 1k → 10k → 100k attempts","PMD SAST integrated"], githubUrl:"https://github.com/farouq7assan0o/SecureCoding" },
  { id:"otp-s3", title:"Secure OTP Data Retrieval System", category:"cloud", year:"2025", tags:["S3 Security","OTP Hardening","OWASP A01/A07","IAM","Presigned URLs"], summary:"Security design review of an OTP-based S3 system. Found and fixed 4 critical vulns: OTP in API response, no expiry, no rate limiting allowing brute-force, predictable S3 path. Fixed with hashed OTPs, 5-min expiry, lockout, uuid4 filenames, private bucket + presigned URLs.", highlights:["OTP in API response → removed, server-side only","Expiry: 5-min window + single-use invalidation","Rate limit: MAX_ATTEMPTS=5 + exponential backoff","S3: uuid4() + private bucket + presigned URL (600s)","OWASP A01, A05, A07 mapped and remediated"], githubUrl:null },
  { id:"apache-ssh", title:"Apache & SSH Hardening (CIS-Aligned)", category:"cloud", year:"2025", tags:["Apache 2.4","CIS Benchmark","SSH Hardening","Docker","Defense-in-Depth"], summary:"Two-part hardening: Apache audit found 5 misconfigs (version disclosure, TRACE enabled, /server-status public, directory listing). Applied CIS 2.2–3.5. SSH: disabled root login + password auth, key-only, removed weak ciphers.", highlights:["Apache: 5 CIS findings fixed","ServerTokens Prod + TraceEnable Off + -Indexes","/server-status → 403 externally","SSH: PermitRootLogin no + PasswordAuthentication no","Ciphers: chacha20-poly1305 + aes256-gcm only"], githubUrl:null },
  { id:"network", title:"Enterprise Network Security Design", category:"cloud", year:"2023", tags:["Cisco","IPsec VPN","OSPF","ASA Firewall","VLAN","ACL"], summary:"Multi-site network across 5 locations. IPsec VPN (AES+SHA), ASA firewall with DMZ and NAT, full-mesh OSPF routing, VLAN segmentation, port security, AAA + SSH-only access. All connectivity, VPN tunnels, and ACLs tested and validated.", highlights:["IPsec VPN: AES + SHA + ISAKMP","ASA: DMZ + NAT + HTTPS-only external","Full-mesh WAN: 5 routers","VLAN isolation per department"], githubUrl:null },
  { id:"governance", title:"BazaarJo Governance Gap Assessment", category:"compliance", year:"2025", tags:["ISO 27014","PDPL","PCI DSS","SoD","Board Brief"], summary:"Post-breach governance assessment. Identified 7 deficiencies including absent SoD (devs deploying to prod unilaterally), undefined CISO accountability, no breach notification policy, missing PDPL compliance. Delivered 12-month remediation roadmap and executive board brief.", highlights:["7 governance gaps across 3 frameworks","SoD: devs had unilateral prod deployment","No PDPL breach notification → regulatory exposure","Board brief: root causes + 3–6 month plan"], githubUrl:null },
  { id:"risk-mgmt", title:"BazaarJo Enterprise Risk Management Plan", category:"compliance", year:"2025", tags:["ISO 27005","NIST 800-30","Risk Register","KRIs","If-Then Risk Statements"], summary:"Full ISO 27005 + NIST 800-30 risk lifecycle. Identified 6 risks rated by likelihood × impact, produced If-Then risk statements, defined KRIs (MFA coverage %, unapproved deploys, revocation lag). Weekly → monthly → quarterly board reporting cadence.", highlights:["Customer PII: Critical (High × High impact)","If-Then: weak git controls → malicious code injection","KRIs: MFA %, unapproved deploys, time-to-revoke","Quarterly board-level risk reporting defined"], githubUrl:null },
  { id:"bia", title:"BazaarJo Business Impact Analysis", category:"compliance", year:"2025", tags:["BIA","RTO/RPO","PCI DSS","Disaster Recovery","Business Continuity"], summary:"Full BIA across 6 critical processes after a 2-hour outage. Payment Processing: RTO 15min/RPO 0–5min (PCI-DSS). Incident Response: RTO 15min/RPO 0 (zero log loss). Online Orders: RTO 30min/RPO 5min.", highlights:["Payment: RTO 15min / RPO 0–5min (PCI-DSS)","IR Monitoring: RTO 15min / RPO 0min","Online Orders: RTO 30min / RPO 5min","Recovery order: Payments → Orders → IR → Support → Catalog"], githubUrl:null },
  { id:"fair", title:"FAIR Risk Analysis — DPSR Phishing Incident", category:"compliance", year:"2024", tags:["FAIR Model","STRIDE/DREAD","Monte Carlo","ISO 27005","PDPL","Jordan Law"], summary:"Quantitative FAIR analysis of a spear-phishing incident (6-day dwell time). Monte Carlo simulation via FAIR-U. ALE before controls: $1.38M/year. After MFA + SIEM + training + IR drill: ALE $177K. Aligned with Jordanian Law No. 16/2019 and PDPL.", highlights:["ALE before: $1.38M/year (Monte Carlo simulation)","ALE after: $177K — $1.2M/year reduction","PDPL: delayed disclosure violated 72hr requirement","Controls: MFA + SIEM + exec phishing training + IR tabletop"], githubUrl:null },
  { id:"isms", title:"ISMS Design — Bluefrontier Bank", category:"compliance", year:"2024", tags:["ISO 27001","COBIT 2019","ISO 27005","BIA","Audit Structure"], summary:"Complete ISMS for Bluefrontier Bank: risk register with KRIs, asset classification, BIA with RTO/RPO, incident lifecycle, 7-phase COBIT map, 3-stage audit. Quantified financial ROI through reduced breach probability. Full ISO 27001 scope.", highlights:["ISO 27001 full scope: policies, controls, audit","COBIT 2019: 7-phase gap → implement → audit","BIA: 6 processes, RTO 15–240min","Executive board brief with financial ROI"], githubUrl:null },
  { id:"crypto", title:"Applied Cryptography — MITM, ECB/CBC, RSA", category:"compliance", year:"2024", tags:["Python","2-DES MITM","RSA","ECB vs CBC","Digital Signatures"], summary:"Four-part project: MITM attack on 2-DES reducing keyspace 2¹¹² → 2⁵⁷; ECB vs CBC comparison showing pattern leakage; hybrid RSA+DES secure messaging with SHA-1 digital signatures; Square-and-Multiply modular exponentiation.", highlights:["MITM: 2¹¹² → 2⁵⁷ keyspace reduction","ECB: pattern leakage visualized on encrypted images","RSA+DES hybrid: session key + data encryption","Square-and-Multiply: O(log e) RSA exponentiation"], githubUrl:null },
  { id:"hopechain", title:"HopeChain — Blockchain Donation DApp", category:"other", year:"2024", tags:["Solidity","Ethereum","Ethers.js","Smart Contract Security"], summary:"Ethereum donation platform with on-chain ledger. Analyzed reentrancy, Sybil attacks, donor address privacy leakage. Proposed zkSNARK privacy and multisig + timelock governance. Jordan NGO regulatory analysis included.", highlights:["Reentrancy mitigated via transfer() pattern","Privacy: donor addresses public → zkSNARK proposed","Jordan regulatory: NGO + crypto banking constraints","Proposed: multisig + timelocks + pausable contract"], githubUrl:null },
  { id:"spark", title:"SPARK — Wearable INR Monitoring Patch", category:"other", year:"2025", tags:["Team Leadership","Biomedical","IoT","Market Validation","HIPAA"], summary:"Led 6-person cross-disciplinary team building a wearable INR monitoring patch for post-stroke patients. Electrochemical sensors + Bluetooth 5.0 + mobile app + AI-powered emergency alerts. Validated $1.65M SOM in Jordan's $55M rehabilitation device market.", highlights:["Led 6-person cross-disciplinary team","$1.65M SOM validated in $55M Jordan rehab market","Non-invasive continuous INR monitoring","AI emergency alerts: patient + caregiver + doctor"], githubUrl:null },
];
...
function Divider({ light = false }) {
  return <div style={{ height: "1px", background: light ? "rgba(255,255,255,0.08)" : T.creamDark, margin: "0 0 24px" }} />;
}
...
function GhostBtn({ onClick, children, active = false, color = T.navy }) {
  const [h, setH] = useState(false);
  return (
    <button onClick={onClick} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ padding: "7px 16px", background: active ? color : h ? color + "12" : "transparent", border: `1.5px solid ${active ? color : h ? color + "60" : T.creamDark}`, color: active ? T.white : h ? color : T.slate, fontFamily: T.body, fontSize: "0.78rem", fontWeight: 500, cursor: "pointer", transition: "all 0.18s", letterSpacing: "0.02em" }}>
      {children}
    </button>
  );
}
...
function Cursor() {
  ...
  // mix-blend-mode: difference = always visible on any bg color
  return (
    <>
  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 500, background: bg, backdropFilter: scrolled ? "blur(14px)" : "none", borderBottom: scrolled ? `1px solid ${T.creamDark}` : "none", transition: "all 0.35s ease", padding: "0 clamp(18px,5vw,60px)" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto", height: "66px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div className="desk-nav" style={{ display: "flex", gap: "32px" }}>
          {items.map(s => (
            <button key={s} onClick={() => go(s)} data-hover
              style={{ background: "none", border: "none", cursor: "pointer", fontFamily: T.body, fontSize: "0.8rem", fontWeight: active === s ? 600 : 400, color: active === s ? (scrolled ? T.navy : (darkBg ? T.white : T.navy)) : (scrolled ? T.slate : (darkBg ? "rgba(255,255,255,0.6)" : T.slate)), letterSpacing: "0.06em", textTransform: "uppercase", position: "relative", padding: "4px 0", transition: "color 0.3s" }}>
              {s}
        ...
        <div className="about-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "60px", alignItems: "start" }}>
          ...
          <Reveal delay={220}>
            {[["Education","B.Sc. Cybersecurity — HTU · Jun 2026"],["Internship","SCC – Jordan Armed Forces · Oct 2025 – Jun 2026 (8 months)"],["Focus","SOC / Detection Engineering + Penetration Testing"],["Platform","HackTheBox — CDSA ✓, CWES 70%, CPTS 45%"],["Competition","Top 10 / 300+ — NCSCJO National Cybersecurity Bootcamp"],["Email",PROFILE.email],["Languages","Arabic (native) · English (proficient) · German · Italian (beginner)"]].map(([k, v]) => (
              <div key={k} style={{ display: "flex", gap: "18px", padding: "13px 0", borderBottom: `1px solid ${T.creamDark}` }} className="kv-row">
...
function ProjectCard({ project }) {
  ...
  return (
    ...
    >
      ...
      {/* Expandable highlights — smooth height transition */}
...
function PhaseBlock({ phase }) {
  ...
  return (
    <div style={{ border: `1px solid ${isActive ? T.orange + "30" : "rgba(255,255,255,0.07)"}`, marginBottom: "8px", transition: "border-color 0.2s" }}>
      <div onClick={() => setOpen(o => !o)} data-hover
            ...
            <Reveal key={c.id} delay={i * 70}>
              <TiltCard style={{ padding: "22px 24px", cursor: c.badgeUrl ? "pointer" : "default", borderLeft: `3px solid #22C97A`, borderTop: `1px solid ${T.creamDark}`, borderRight: `1px solid ${T.creamDark}`, borderBottom: `1px solid ${T.creamDark}` }} onClick={() => c.badgeUrl && window.open(c.badgeUrl, "_blank")}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "10px", gap: "8px" }}>
                  ...
                  <span style={{ fontSize: "0.58rem", fontFamily: T.mono, color: "#22C97A", background: "#22C97A18", border: "1px solid #22C97A60", padding: "3px 9px", letterSpacing: "0.08em", whiteSpace: "nowrap", borderRadius: "2px" }}>✓ EARNED</span>
                </div>
                ...
                {c.badgeUrl && <div style={{ marginTop: "12px", fontFamily: T.mono, fontSize: "0.65rem", color: "#22C97A", display: "flex", alignItems: "center", gap: "4px" }}>verify badge ↗</div>}
              </TiltCard>
            </Reveal>
          ))}
        </div>
            ...
            <Reveal key={c.id} delay={i * 80}>
              <div style={{ padding: "22px 24px", background: T.white, borderLeft: `3px solid #F0A830`, borderTop: `1px solid ${T.creamDark}`, borderRight: `1px solid ${T.creamDark}`, borderBottom: `1px solid ${T.creamDark}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px", gap: "8px" }}>
                  ...
                  <span style={{ fontSize: "0.58rem", fontFamily: T.mono, color: "#F0A830", background: "#F0A83018", border: "1px solid #F0A83060", padding: "3px 9px", letterSpacing: "0.08em", whiteSpace: "nowrap", borderRadius: "2px" }}>IN PROGRESS</span>
                </div>
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: ${T.navy}; } ::-webkit-scrollbar-thumb { background: ${T.orange}; border-radius: 2px; }
      ...
      <Cursor />
      <Nav active={active} setActive={setActive} darkBg={["projects","roadmap","writeups"].includes(active)} />
